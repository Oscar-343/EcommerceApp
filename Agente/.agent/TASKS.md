...
- **`GuideEdit`/`TransportEdit` pisan `CreatedAt`** al editar (mismo bug que tenía `ProductEdit`/`ServiceEdit` antes del refactor, que sí se corrigió). Nadie lo ha corregido todavía porque ningún plan lo mencionó explícitamente. Ver detalle en `Agente/.agent/MEMORY.md`.
